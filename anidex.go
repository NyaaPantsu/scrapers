...
func anidexChild(chTorrent chan<- Torrent, chPageID chan string) {
	...
	for pageID := range chPageID {
		...
		req, err := http.NewRequest("GET",
			"https://anidex.info/torrent/"+pageID, nil)
		if err != nil {
			fmt.Println(err)
		}
		...
		info.Source = "https://anidex.info/torrent/" + pageID
		...
		info.Hash = strings.ToUpper(strings.TrimSpace(doc.Find("#edit_torrent_form > div:nth-child(1) > div:nth-child(2) > table:nth-child(1) > tbody:nth-child(2) > tr:nth-child(7) > td:nth-child(2)").Text()))
